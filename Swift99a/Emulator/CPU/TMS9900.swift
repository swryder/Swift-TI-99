...
final class TMS9900: Peripheral {
    ...
    func wrword(D dest: UInt16, V val: UInt16, rmw: MemoryAccess = .write) {
        ...
        core.writeMemoryByte(address: d + 1, cycles: &nCycleCount, accessType: rmw, data: UInt8(val & 0xFF))
        core.writeMemoryByte(address: d, cycles: &nCycleCount, accessType: rmw, data: UInt8((val >> 8) & 0xFF))
    }
