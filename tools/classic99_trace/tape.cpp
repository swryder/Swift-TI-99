...
#include "cassette_trace.h"   // [trace] cassette decode trace logger
...
bool getTapeBit() {
    ...
    if ((tape_pos < tape_buf_size) && (pTapeBuf != NULL)) {
        sampleVal = pTapeBuf[tape_pos];
        ret = (sampleVal >= cutoff);
    }
    // [trace] log every CDIN read with tape position, raw sample, and bit
    casTrace("CDIN pos=%d sample=%d bit=%d motor=%d", tape_pos, sampleVal,
             ret ? 1 : 0, tape_motor_on ? 1 : 0);
...
void setTapeMotor(bool isOn) {
    ...
    // [trace] open the trace file and log every motor state change
    casTraceOpen();
    casTrace("MOTOR req=%d prevOn=%d state=%d pos=%d size=%d",
             isOn ? 1 : 0, tape_motor_on ? 1 : 0, tape_motor_state,
             tape_pos, tape_buf_size);
    if (!isOn) {
        ...
        if (tape_motor_state == 1) {
            tape_motor_on = true;
            // [trace] this is the moment the tape REALLY starts playing
            // (the off→on transition that ungates the decoder). Reset the
            // trace clock so the 60s cap covers actual decoding only.
            casTraceResetClock();
...
void LoadTape() {
    ...
    // [trace] log first 16 post-processed samples — useful to confirm the
    // WAV pipeline produces the same shape Swift99a is feeding the ROM.
    {
        char first[80] = {0};
        for (int j = 0; j < 16 && j < samplecount; ++j) {
            char tmp[8];
            sprintf(tmp, "%02X ", pTapeBuf[j]);
            strcat(first, tmp);
        }
        casTrace("LOAD avg=%d scale=%.2f size=%d first=%s",
                 avg, scale, samplecount, first);
    }
    free(buf);
