...
// [trace] cassette decode trace logger — see tools/classic99_trace/
extern "C" void casTrace(const char* fmt, ...);
...
// 				debug_write("9901 timer expired, int requested");
				timer9901IntReq=1;
				// [trace] timer expiration — log start value (= period)
				casTrace("TIMERFIRE start=%d", starttimer9901);
			}
...
//			if (cycles_left >= 22) {					// speed throttling
				// [trace] log INT1 entry — capture interrupted PC/WP/ST
				// and which source pulled the line (VDP or timer or both).
				casTrace("INT1ENTRY pc=%04X wp=%04X st=%04X src=%s%s",
				         pCurrentCPU->GetPC(), pCurrentCPU->GetWP(), pCurrentCPU->GetST(),
				         (VDPINT && CRU[2]) ? "V" : "",
				         (timer9901IntReq && CRU[3]) ? "T" : "");
...
//                        debug_write("9901 timer now set to %d", starttimer9901);
                    ...
                    // [trace] log every set-bit during timer-load LDCR
                    casTrace("CRUWRITE op=SBO bit=%d clockmode=1 starttimer=%d",
                             ad, starttimer9901);
                    if (starttimer9901 != 0) {
				...
				switch (ad) {
					...
					case 3:
						// timer interrupt bit
                        // [trace] timer ack via SBO 3 (write 1 to bit 3 in I/O mode)
                        casTrace("TIMERACK op=SBO had=%d", timer9901IntReq ? 1 : 0);
                        if (timer9901IntReq) {
    						timer9901IntReq=0;
...
//                        debug_write("9901 timer now set to %d", starttimer9901);
				...
				switch (ad) {
					case 3:
						// timer interrupt bit
                        // [trace] timer ack via SBZ 3 (write 0 to bit 3 in I/O mode)
                        casTrace("TIMERACK op=SBZ had=%d", timer9901IntReq ? 1 : 0);
                        if (timer9901IntReq) {
    						timer9901IntReq=0;
